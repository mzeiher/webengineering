...
export async function getDBConnection(dir: string): Promise<DB> {
    const db = await new Promise<sqlite3.Database>((resolve, reject) => {
        const db = new sqlite3.Database(dir, sqlite3.OPEN_CREATE | sqlite3.OPEN_READWRITE, (err) => {
            if (err) {
    ...
    return {
        async add(todo: string) {
            const statement = new Promise<void>((resolve, reject) => {
                db.run(`INSERT INTO todos (todo, isDone) VALUES (?, false)`, [todo], (err) => {
                    if (err) {
                        reject(err);
                        return;
                    }
        ...
        async updateTodo(id: number, text: string, isDone: boolean) {
            const statement = new Promise<void>((resolve, reject) => {
                db.run(`UPDATE todos SET todo = ?, isDone = ? WHERE id = ?`, [text, isDone, id], (err) => {
                    if (err) {
                        reject(err);
                        return;
                    }
