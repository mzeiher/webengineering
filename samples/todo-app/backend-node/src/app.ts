...
const STATIC_DIR = path.resolve(path.parse(fileURLToPath(import.meta.url)).dir, "static");
const DB_DIR = path.resolve(path.parse(fileURLToPath(import.meta.url)).dir, "db");
...
fastify.get('/*', async (request, reply) => {
    const requested_resource = request.url === '/' ? 'index.html' : request.url.substring(1); // trim leading slash
